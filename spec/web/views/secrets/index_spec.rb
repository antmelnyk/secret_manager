RSpec.describe Web::Views::Secrets::Index, type: :view do
    ...
    it 'exposes #secrets' do
        expect(view.format).to eq exposures.fetch(:secrets)
	end
	...
	context 'when there are secrets' do
		let(:secret1)     { Secret.new(login: 'Admin', secret: Encryptor.generate('secret1')) }
		let(:secret2)     { Secret.new(login: 'Superadmin', secret: Encryptor.generate('secret2')) }
